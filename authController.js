...
class authController {
    ...
    async postBasket(req, res) {
        try {
            ...
            const card = await Basket.create({ name, description, price, img, basketImg, isToggle })
            await card.save()
    ...
    async deleteCard(req, res) {
        try {
            const { id } = req.params;
            const edit = await MostSaledItems.findByIdAndUpdate(id, { isToggle: false });
    ...
    async editBasket(req, res) {
        try {
            const { id } = req.params
            const _id = id
            const editedCard = await MostSaledItems.findByIdAndUpdate(_id, { isToggle: true })
    ...
    async gadjetPut(req, res) {
        try {
            ...
            switch (name) {
                case 'Phones':
                    response = await Phone.findByIdAndUpdate({ _id }, { name: itemName, description: description, img: img, price: price })
                    break;
                case 'Laptops':
                    response = await Laptop.findByIdAndUpdate({ _id }, { name: itemName, description: description, img: img, price: price })
                    break;
                case 'Watches':
                    response = await Watch.findByIdAndUpdate({ _id }, { name: itemName, description: description, img: img, price: price })
                    break;
                case 'Ipads':
                    response = await Ipad.findByIdAndUpdate({ _id }, { name: itemName, description: description, img: img, price: price })
                    break;
            }
            res.json(response);
        } catch (e) {
    ...
    async gadjetPost(req, res) {
        try {
            ...
            switch (name) {
                case 'Phones':
                    response = await Phone.create({ name: itemName, description: description, img: img, price: price })
                    await response.save()
                    break;
                case 'Laptops':
                    response = await Laptop.create({ name: itemName, description: description, img: img, price: price })
                    await response.save()
                    break;
                case 'Watches':
                    response = await Watch.create({ name: itemName, description: description, img: img, price: price })
                    await response.save()
                    break;
                case 'Ipads':
                    response = await Ipad.create({ name: itemName, description: description, img: img, price: price })
                    await response.save()
                    break;
            }
            res.json(response);
    ...
    async changePassword(req, res) {
        try {
            ...
            const editedPassword = await User.findByIdAndUpdate({ _id: decoded.id }, { password: hashNewPassword })
    ...
    async editUserImg(req, res) {
        try {
            ...
            let decoded = jwt_decode(token)
            const editedUserImg = await User.findByIdAndUpdate({ _id: decoded.id }, { img: img })
